...
/*void sx126x_read_register3(  const uint16_t address, uint8_t* buffer, const uint8_t size )
{
    const uint8_t buf[4] = {
        0x1D,
    ...
    sx126x_hal_read3(  buf, 4, buffer, size );
}

void sx126x_hal_read3( const uint8_t* command, const uint16_t command_length,
                                     uint8_t* data, const uint16_t data_length )
{

    // Wait for readiness
    sx126x_hal_check_device_ready3(  );
...
void sx126x_hal_check_device_ready3(  )
{
    if( radio_mode != RADIO_SLEEPRA )
    {
        sx126x_hal_wait_on_busy3(3 );
    }
    else
    {
        ...
        sx126x_hal_wait_on_busy3( 3 );
        gpio_set_level(7, 1);
        radio_mode = RADIO_AWAKERA;
    }
}

void sx126x_hal_wait_on_busy3( const int busy_pin )
{
    //while( hal_gpio_get_value( busy_pin ) == HAL_GPIO_SET )
    while (gpio_get_level(busy_pin) == 1)
    {
        ESP_LOGE(TAG, "WaitForIdle ");
    };
}



void get_SpiHandle(spi_device_handle_t* spihandle) {
	*spihandle = &SpiHandle;
}

void set_SpiHandle(spi_device_handle_t* spihandle) {
	SpiHandle = *spihandle;
}*/
